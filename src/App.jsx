...
const App = () => {
  ...
  const features = [
    ...
    },
    {
      ...
      desc2:
        "외부 서비스와 내부 관리 시스템 등 다양한 산업군의 프로젝트를 수행하며 UI/UX 특성과 퍼블리싱 요구사항을 이해하고 있습니다. 사용자 화면부터 관리자 페이지까지 다양한 형태의 화면을 구현하며, 데이터 테이블과 폼 구조가 복잡한 화면에서도 일관성과 사용성을 고려한 퍼블리싱을 진행했습니다.",
      tags: ["Admin Dashboard", "Public Platform", "Education", "Projects"],
    },
    {
      title: "반응형과 접근성 완벽 구현!",
      desc: "모바일과 데스크탑 모두 사용자 친화적인 UI를 구현합니다.",
      desc2:
        "WCAG 2.1 가이드라인을 준수하여 시각, 청각, 운동 장애를 가진 사용자도 불편함 없이 이용할 수 있는 웹을 만듭니다. Semantic HTML을 활용한 스크린 리더 지원, 키보드 네비게이션 최적화, 적절한 색상 대비 등을 고려하며, 모든 디바이스에서 최상의 경험을 제공합니다.",
      tags: [
        "WCAG 2.1",
        "Semantic HTML",
        "Mobile First",
    ...
    },
    {
      ...
      desc2:
        "Figma, Zeplin을 통해 디자인을 확인하고 퍼블리싱 작업을 진행했으며, Git을 사용해 소스 관리와 협업을 경험했습니다. Jira, Confluence를 활용해 작업 현황을 공유했고, 기획자 및 디자이너와의 협업 과정에서 이해하기 쉬운 소통을 중요하게 생각하며 업무를 진행했습니다.",
      tags: [
        "Figma",
        "Zeplin",
        "Git",
    ...
    },
    {
      ...
      desc2:
        "새로운 기술이나 라이브러리를 사용할 때 공식 문서를 먼저 참고하며 필요한 부분을 학습해왔습니다. 버그나 이슈가 발생하면 Chrome DevTools로 원인을 확인하고, 필요한 경우 Stack Overflow나 GitHub Issues를 찾아보며 해결했습니다. 해결한 내용은 간단히 정리해 팀원들과 공유하며, 이후 비슷한 문제가 다시 발생하지 않도록 참고 자료로 활용했습니다.",
      tags: [
        "Chrome DevTools",
        "Debugging",
        "Documentation",
    ...
    },
    {
      ...
      desc2:
        "사용자 흐름과 화면 사용 방식을 고려하며 퍼블리싱 작업을 진행했습니다. 로딩 속도, 에러 메시지 표현, 접근성 등 사용자 경험에 영향을 주는 요소들을 점검하고, 개선이 필요한 부분은 기획자·개발자와 공유했습니다. 주어진 디자인을 그대로 구현하는 데 그치지 않고, “왜 이런 구조인지”를 고민하며 더 나은 화면 구성 방향을 함께 논의해왔습니다.",
      tags: ["Performance", "UX Research", "Testing"],
    },
    {
      title: "효율적인 작업 방식",
      desc: "템플릿 활용, 컴포넌트 재사용, SCSS 변수 관리 등 효율적인 퍼블리싱을 지향합니다.",
      desc2:
        "재사용 가능한 컴포넌트 라이브러리를 구축하여 개발 속도를 높이고, SCSS의 mixin, function, variable을 활용한 체계적인 스타일 관리로 유지보수성을 극대화합니다. BEM 방식을 적용하여, 클래스 네이밍과 마크업의 일관성을 중요하게 생각하며 작업했습니다.",
      tags: ["BEM", "SCSS", "Component", "Prettier", "ESLint"],
    },
  ];
